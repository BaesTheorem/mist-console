...
**Fable only orchestrates.** A chat on any Fable model gets `bridge.FABLE_ORCHESTRATOR_PROMPT` appended (plan, delegate, check, reply; no `fork` subagents, since forks inherit the parent's model) and spawns with `CLAUDE_CODE_SUBAGENT_MODEL=claude-opus-5-5[1m]` plus `CLAUDE_CODE_SUBAGENT_MODEL_FORCE=1`, which makes the CLI ignore any per-call `model` override on Agent and Workflow subagents (checked live 2026-09-24: an Agent call asking for `opus` ran on the pinned model with FORCE set, and on Opus without it). Both are fixed at spawn, so a model switch into or out of Fable restarts the backend with `--resume` instead of switching live. `run-routine.sh` and the Windows bridge apply the same rule.
